...
export const Pedido = () => {
  ...
  const getData = async () => {
    ...
    const estoqueItems = await supabase.from("estoque").select().eq("id_empresa", equipe.empresas).order("item");

    const favorecidos = await supabase.from("favorecido").select().eq("id_empresa", equipe.empresas).order("favorecido");
  ...
  const handleRegister = async (e: any) => {
    ...
    if (swalAlert.isConfirmed) {
      ...
      const findItem = data?.estoqueItems?.find((item) => Number(item.id) === Number(fields.id_item));
  ...
  return (
    <div className={styles.panel}>
      <header className={styles.stockPanelHeader}>
        <h1>
          <MdArrowBack className={styles.arrowBack} onClick={() => navigate("/historico")} />
      ...
      <div className={styles.content}>
        <form className={styles.form} onSubmit={handleRegister}>
          <div className={styles.inputGroup}>
            ...
            <select name="id_item" id="id_item" onChange={changeField} defaultValue="">
          ...
          <div className={styles.inputGroup}>
            ...
            <select name="tipo" id="tipo" onChange={changeField} value={fields.tipo}>
          ...
          {fields.tipo === "Entrada" ? null : (
            <div className={styles.inputGroup}>
              ...
              <select name="id_favorecido" id="id_favorecido" onChange={changeField} defaultValue="">
                <option value="" disabled></option>
          ...
          <div className={styles.inputGroup}>
            ...
            <input type="text" name="quantidade" id="quantidade" onChange={changeField} value={fields.quantidade} />
          </div>
